...
6. SIZE AND SCOPE: ~100 lines changed is reviewable; ~300 acceptable for one logical change; ~1000 must be split. SCOPE: the diff must contain only this task's changes — no unrelated refactors, no "while I'm here" fixes, no bonus cleanups riding along. The full discipline is rigor-scope (DECLARE the boundary before coding; TOUCH AUDIT before commit; revert what is out of scope); this review enforces it. Separate refactoring from feature work — never both in one change.
